...
**Feedback channel:** Settings → Help Us Improve → `FeedbackView` (lives in SupportView.swift): topic chips (Idea / Something Isn't Working / General Feedback) + message box → composes a pre-filled email to support@littlelightsbiblebedtime.com via `mailto:` (parent-initiated, app+iOS version appended, graceful alert when no mail app). COPPA-clean — nothing automatic.

...
5. **pbxproj is objectVersion 56** (explicit file refs). Every new .swift file needs 4 insertions: PBXBuildFile, PBXFileReference, group children, Sources phase. Established ID pattern `E0ABCDEF1234567890ABCC#` / `E01234567890ABCDEF1234C#` — last used **CD** (CC GameContent, CD BibleGames; C7–CA are the four Journey files). Next: CE. (The Wind-Down/Parent-Voice files used one-off `…D11`/`F2…` IDs — harmless, but stick to the C-series.) (Appending a struct to an existing registered file avoids this entirely.)

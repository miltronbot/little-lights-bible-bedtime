...
**Games tab** (`GamesView` hub + `Views/Components/BibleGames.swift` + content banks in `Models/GameContent.swift` — all offline/procedural, every ending encouraging, all content rooted in the app's stories): **seven games** in three sections, all awarding shared game stars (`GameStars.award` → `games.totalStars`, shown as the hub's counter chip). Guess & Remember: Story Quiz (6/round from a 60-question bank), Who Am I? (16 character riddles, 3 clues each — earlier guess = more stars), Lumi's True or False (28 true/false items, each with a gentle teaching note; renamed from "True or Lumi?" June 2026 — owner: never imply Lumi means false), Guess the Story (story artwork behind blur, clear it in steps — blurrier guess = more stars). Match & Order: Treasure Match (Cozy 6-pair / Big Kid 8-pair difficulties, best-flips in `game.memoryMatch.bestMoves`), Story Scramble (10 stories × 4 events, tap in order). Learn the Verse: Verse Builder (tap word chips to assemble a `memoryVerse` — matching is by TEXT not chip id, verses repeat "the"), plus the Verse Practice door into `MemoryVerseGameView`. Keep new content unambiguous and gentle — no trick questions, no sting.

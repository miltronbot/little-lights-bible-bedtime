...
- **Stories re-rooted in Scripture (June 2026 overhaul):** all 50 `storyText`s were rewritten to be faithful retellings of their `bibleReference` passage, ~2.5–3× longer (10.7k→30.6k words total, ~540–700 words each), with **real World English Bible (WEB, public domain) verses quoted and cited** inline. `memoryVerse` is now exact WEB; `takeaway`/`bedtimePrayer`/`talkAboutIt` refreshed to match. The OT divine name "Yahweh" is rendered "the LORD" in all quotes. Every verse was verified against bible-api.com (WEB). Spec: `docs/StoryRewriteSpec.md`. WEB credited in Settings footer. To add/edit a verse, fetch exact text: `curl -s "https://bible-api.com/<book>+<chap>:<verse>"`.
...
- **Audio — ⚠️ REGEN PENDING:** All 50 MP3s are bundled (not streamed). After the June 2026 story rewrite, the bundled MP3s still narrate the OLD short text — **they must be regenerated** so narration matches (and read-along, which is proportional to playback, stays in sync). Owner step: `python scripts/generate_audio.py` with an OpenAI key (~$3; narrates title + storyText + bedtimePrayer). `AVAudioSession` configured as `.playback, mode: .default` at startup AND inside each `loadAudio` call
